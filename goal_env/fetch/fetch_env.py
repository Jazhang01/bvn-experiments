...
class FetchEnv(robot_env.RobotEnv):
    ...
    def __init__(
            ...
            goal_sampling_type, far_goal_threshold,
        ...
        self.far_goal_threshold = far_goal_threshold
    ...
    def _sample_goal(self, goal_sampling_type=None):
        if goal_sampling_type is None:
            goal_sampling_type = self.goal_sampling_type
        
        if goal_sampling_type == 'default':
        ...
        elif goal_sampling_type == 'near':
            goals = self._sample_goal_by_distance(max_distance=self.far_goal_threshold)
        elif goal_sampling_type == 'far':
            goals = self._sample_goal_by_distance(min_distance=self.far_goal_threshold)
        elif goal_sampling_type == 'left':
            goals = self._sample_goal_by_left_or_right(use_left=True)
        elif goal_sampling_type == 'right':
            goals = self._sample_goal_by_left_or_right(use_left=False)

        return goals[self.goal_key] if isinstance(self.goal_key, str) else goals

    def _sample_goal_by_left_or_right(self, use_left):
        if self.goal_key == "robot0:grip":
            if use_left:
                return self.initial_gripper_xpos[:3] + \
                        self.np_random.uniform((-self.target_range, 0, -self.target_range), (self.target_range, self.target_range, self.target_range), size=3)
            else:
                return self.initial_gripper_xpos[:3] + \
                        self.np_random.uniform((-self.target_range, -self.target_range, -self.target_range), (self.target_range, 0, self.target_range), size=3)
    ...
    def _sample_goal_by_distance(self, min_distance=1e-9, max_distance=1e9):
        # import ipdb; ipdb.set_trace()
        # Just need a dummy goal for get_obs to work
        self.goal = goal = self._sample_goal(goal_sampling_type='default')
        obs = self._get_obs()
        dist = goal_distance(obs['achieved_goal'], goal)
        while dist is None or (dist <= min_distance) or (dist >= max_distance):
            goal = self._sample_goal(goal_sampling_type='default')
            obs = self._get_obs()
            dist = goal_distance(obs['achieved_goal'], goal)
