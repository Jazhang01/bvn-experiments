...
class MazeEnv(gym.Env):
    ...
    def _is_in_collision(self, pos):
        ...
        for i in range(len(structure)):
            for j in range(len(structure[0])):
                if structure[i][j] == 1:
                    minx = (j - 0.5) * size_scaling - self._init_torso_x
                    maxx = (j + 0.5) * size_scaling - self._init_torso_x
                    miny = (i - 0.5) * size_scaling - self._init_torso_y
                    maxy = (i + 0.5) * size_scaling - self._init_torso_y
